...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        for handLms in results.multi_hand_landmarks:
            ...
            x = int(middle_finger_knuckle.x*len(depth_image[0]))
            y = int(middle_finger_knuckle.y*len(depth_image))
            if x >= len(depth_image[0]):
                x = len(depth_image[0]) - 1
            if y >= len(depth_image):
                y = len(depth_image) - 1
            mfk_distance = depth_image[y,x] * depth_scale # meters
